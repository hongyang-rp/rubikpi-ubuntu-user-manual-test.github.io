...
3. 从 [Canonical 网站](https://ubuntu.com/download/qualcomm-iot) 下载 `dtb.bin` 并复制到 rb3_ubuntu_images 文件夹中。

4. 从 [Canonical 网站](https://ubuntu.com/download/qualcomm-iot) 下载 `ubuntu-24.04-preinstalled-server-arm64+rubikpi3-20250912-127.img.xz`，解压缩包以获得原始镜像文件。
...
5. 将 Ubuntu原始映像 `ubuntu-24.04-preinstalled-server-arm64+rubikpi3-20250912-127.img` 文件复制到 `rubikpi3_Ubuntu_images` 文件夹。
    ...
    ```
6. 从 [Canonical 网站](https://ubuntu.com/download/qualcomm-iot) 下载 `rawprogram0.xml` 并复制到 `rubikpi3_ubuntu_images` 文件夹中。
