...
1. 如果 gst pipeline 状态更改为 “PLAYING”，如下所示，则表示摄像头正在运行。由于此命令会将摄像头帧 dump 到 fakesink，因此不会在设备上保存任何内容。
...
1. 参阅 [摄像头排线安装](3.peripherals%20and%20interfaces.md#cameracable)检查摄像头模块连接。
