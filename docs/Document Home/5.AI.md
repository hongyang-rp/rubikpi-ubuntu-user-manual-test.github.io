...
■ Qualcomm Kryo TM CPU- 一流的 CPU ,具有高性能和卓越的能效。
...
##### 5.3.1.1.1 在您的计算机上安装miniconda和配置Python环境。
...
##### 5.3.1.1.2 安装 AI Hub Python 客户端。
...
登录后,导航至 Account > Settings > API Token 。此时应提供一个可用于配置客户端的 API

令牌。



##### 5.3.1.1.4 在终端,使用以下命令通过 API 令牌配置客户端。
...
1. 导航到 AI Hub Model Zoo ,访问适用于 RUBIK Pi 3的预优化模型。
...
参见快速入门中"运行示例应用程序"中内容，确保里面的的示例程序可以正常运行。



注：桌面版本需要使用如下方式启动weston
...
##### 5.4.1.2 模型文件和label文件下载



**LiteRt的示例程序名称和对应的模型文件，label文件列表：**
...
* 点击下载示例程序需要的模型文件和label文件的压缩包ai\_sample\_app\_model\_label.zip。
...
#### 5.4.2.1 通过摄像头获取图像信息实现ai功能的示例程序

##### 5.4.2.1.1 图像分类(image-classification-LiteRT-from-camera)
...
##### 5.4.2.1.2 目标检测(object-detection-LiteRT-from-camera)
...
##### 5.4.2.1.3 语义分割(image-segmentation-LiteRT-from-camera)
...
##### 5.4.2.1.4 姿态识别(pose-detection-LiteRT-from-camera)
...
#### 5.4.2.2 通过录制好的mp4文件获取图像信息实现ai功能的示例程序

##### 5.4.2.2.1 图像分类(image-classification-LiteRT-from-file)
...
##### 5.4.2.2.2 目标检测(object-detection-LiteRT-from-file)
...
##### 5.4.2.2.3 语义分割(image-segmentation-LiteRT-from-file)
...
##### 5.4.2.2.4 姿态识别(pose-detection-LiteRT-from-file)
...
##### 5.4.3.1 图像分类(image-classification-LiteRT-from-camera)
...
##### 5.4.3.2 目标检测(object-detection-LiteRT-from-camera)
...
#### 5.4.3.1 通过录制好的mp4文件获取图像信息实现ai功能的示例程序

##### 5.4.3.1.1 图像分类(image-classification-LiteRT-from-file)
...
##### 5.4.3.1.2 目标检测(object-detection-LiteRT-from-file)
