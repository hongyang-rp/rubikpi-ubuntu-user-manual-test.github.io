...
## 更新内核
您在使用 `sudo apt upgrade` 命令更新时会自动将您的内核更新到最新的稳定版本。如果您想尝试最新的不稳定测试内核，可以 [下载最新内核源码](#下载内核源代码) 并 [手动编译和更新内核](#Native_and_cross_builds)。
...
```
接下来，[下载你的内核源代码](#下载内核源代码)。特别要注意的是，确保您已经完成了 [本地编译或交叉编译](#Native_and_cross_builds) 的步骤。
