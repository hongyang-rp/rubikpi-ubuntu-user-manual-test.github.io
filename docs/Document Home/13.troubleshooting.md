...
### 如何刷写配置参数表 (CDT)？
  ...
  确保设备处于 EDL 模式。有关更多信息，请参阅 [进入 EDL 模式](./1.quick-start-guide/2.update-software.md#进入-edl-模式)。
  :::
...
### 如何配置通用闪存存储 (UFS)？
  ...
  * 请参阅 [使用 QDL 工具进行刷写](1.quick-start-guide/2.update-software.md#使用-qdl-工具进行烧录)。

### 如何将设备强制进入 EDL 模式？

  * 请参阅 [进入 EDL 模式](1.quick-start-guide/2.update-software.md#进入-edl-模式)。
...
### 如何使用 Minicom 来设置调试 UART？
  ...
  * 请参阅 [设置调试 UART](./1.quick-start-guide/3.set-up-device.md#设置调试-uart).
...
### 如何从桌面版本切换为服务版本
  ...
  1. 参见 [连接到网络](1.quick-start-guide/3.set-up-device.md#连接到网络) , 确保网络连接状态正常。
